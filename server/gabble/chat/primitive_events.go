...
func InitBasicEvents(server *SocketIOServer) {
	server.Socket.OnConnect(RootNamespace, func(so socketio.Conn) error {
		so.SetContext("")
	...
	server.Socket.OnError(RootNamespace, func(conn socketio.Conn, e error) {
	...
	server.Socket.OnDisconnect(RootNamespace, func(conn socketio.Conn, reason string) {
